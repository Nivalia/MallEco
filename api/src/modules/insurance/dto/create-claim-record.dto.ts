import { IsString, IsDate, IsDecimal, IsNumber, IsOptional, IsNotEmpty } from 'class-validator';
import { Type } from 'class-transformer';

export class CreateClaimRecordDto {
  @IsString({ message: '理赔单号必须是字符串' })
  @IsNotEmpty({ message: '理赔单号不能为空' })
  claimNumber: string;

  @IsString({ message: '保单ID必须是字符串' })
  @IsNotEmpty({ message: '保单ID不能为空' })
  policyId: string;

  @IsString({ message: '保险公司ID必须是字符串' })
  @IsNotEmpty({ message: '保险公司ID不能为空' })
  insuranceCompanyId: string;

  @IsString({ message: '投保人ID必须是字符串' })
  @IsNotEmpty({ message: '投保人ID不能为空' })
  ...
  @IsNotEmpty({ message: '理赔类型不能为空' })
  claimType: string;

  @IsString({ message: '理赔原因必须是字符串' })
  @IsNotEmpty({ message: '理赔原因不能为空' })
  claimReason: string;

  @IsString({ message: '事故地点必须是字符串' })
  @IsNotEmpty({ message: '事故地点不能为空' })
